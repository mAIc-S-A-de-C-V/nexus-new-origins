...
- `create_app` — payload **MUST include `name`** at the top of the payload (NOT only on the outer block). Full shape:
  ...
  **ADVANCED widget features** (use these instead of materializing a new object type via Logic Function whenever the goal is visualization):
  - `joins: [{alias, target_object_type_id, on: {source_field, target_field}, type: "left"|"inner"}]` — query-time LEFT/INNER JOIN to another object type. Joined columns are reachable everywhere a field name is expected via `alias.field` (e.g. `emp.full_name`, `proj.name`). Use `target_field: "id"` to join against the joined record's canonical id (resolves to its `source_id` column, the right thing for CSV-imported records that don't carry an inline `id` in JSONB).
  - `computedFields: [{name, expression}]` — per-widget virtual columns. Reference the alias from `valueField`, `labelField`, `field` on aggregations, or filter keys. The `expression` is a JSON AST. Operators: arithmetic `add/sub/mul/div/mod`, comparison `eq/neq/lt/lte/gt/gte`, logical `and/or`, unary `neg/not`. Functions: `concat, lower, upper, coalesce, date_diff(unit,a,b), date_trunc(unit,ts), now(), to_number, to_date, to_text, if(c,t,e), round, abs, floor, ceil, pow, length`. Numeric literals (`30`, `2.5`) and unit literals (`"day"`, `"month"`) are inlined safely. Cyclic references between computed fields are rejected.
  - `window` on an aggregation — running totals, moving averages, lag/lead, rank. Shape: `{partition_by: ["series"], order_by: [{field: "grp", dir: "asc"}], frame_mode: "cumulative"|"rolling"|"all", frame_rows?: N, offset?: N}`. The aggregation's `field` then refers to an inner column: `grp`, `series`, or `agg_N` (alias of a non-windowed earlier aggregation). For cumulative sums use `frame_mode: "cumulative"`; for moving averages use `frame_mode: "rolling"` and set `frame_rows`. For rank/lag/lead use `frame_mode: "all"`.

  **Decision tree:**
  - "show the name not the ID" → add a join, reference `alias.<name_field>` as `labelField`
  - "compute X from raw fields" (e.g. daily_cost from salary + allocation) → declare a computedField with the expression and use its alias as `valueField`
  - "running total / cumulative / moving average / rank by" → add `window` to the aggregation
  - "total spent to date" (one number per group) → computed field that multiplies daily-rate by `date_diff("day", to_date(start_date), now())`, then SUM grouped
  - Combine all three in one widget when the request demands it — one `/aggregate` call covers join + compute + window.

  Worked example — line chart of cumulative project cost per day, joining to Employee for salary and Project for the label:
  ```nexus-action
  {"type":"create_app","name":"Project Cost Accrual","summary":["Per-project daily cost run-rate climbing as new assignments start","Joins Employee for salary, Project for label","Computed daily_cost = (salary/30) * (allocation_pct/100)","Cumulative window per project over time"],"payload":{"name":"Project Cost Accrual","description":"Daily cost run-rate per project, cumulative line per project over time.","kind":"dashboard","object_type_ids":["<ProjectAssignment-id>"],"components":[{"id":"cost-line","type":"line-chart","title":"Cumulative cost per project","objectTypeId":"<ProjectAssignment-id>","xField":"start_date","timeBucket":"day","labelField":"proj.name","valueField":"daily_cost","aggregation":"sum","colSpan":12,"gridX":0,"gridY":0,"gridH":7,"joins":[{"alias":"emp","target_object_type_id":"<Employee-id>","on":{"source_field":"employee_id","target_field":"id"},"type":"left"},{"alias":"proj","target_object_type_id":"<Project-id>","on":{"source_field":"project_id","target_field":"id"},"type":"left"}],"computedFields":[{"name":"daily_cost","expression":{"type":"op","op":"mul","left":{"type":"op","op":"div","left":{"type":"field","name":"emp.monthly_salary"},"right":{"type":"lit","value":30}},"right":{"type":"op","op":"div","left":{"type":"field","name":"allocation_pct"},"right":{"type":"lit","value":100}}}}],"window":{"partition_by":["series"],"order_by":[{"field":"grp","dir":"asc"}],"frame_mode":"cumulative"}}]}}
  ```
  Note in the payload above: the `window` attaches to the aggregation slot on the widget (since line-chart has a single aggregation, it lives at the top level alongside `aggregation`/`valueField`). The full feature reference is in `docs/WIDGET_ADVANCED_FEATURES.md`.

  Do NOT propose creating a new object type or Logic Function when the user asks for a visualization that these primitives can express. Reserve materialization for repeated heavy queries or point-in-time snapshots.
