...
import { PackageOpen, Upload, CheckCircle, AlertCircle, ChevronDown, ChevronUp, Link2 } from 'lucide-react';
...
export const PostmanConnectorModal: React.FC<Props> = ({ onClose }) => {
  ...
  const [tokenMode, setTokenMode] = useState<'static' | 'dynamic' | 'connector'>('static');
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          ...
          {stage === 'variables' && parsed && (
            <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              {/* Auth credential variables notice — config is in the select step */}
              {parsed.credVars.filter((v) => TOKEN_VAR_NAMES.has(v.toLowerCase())).length > 0 && (
                <div style={{
                  padding: '10px 14px', backgroundColor: '#EFF6FF',
                  border: '1px solid #BFDBFE', borderRadius: 8,
                  fontSize: 12, color: '#1E40AF',
                }}>
                  <strong>{parsed.credVars.filter((v) => TOKEN_VAR_NAMES.has(v.toLowerCase())).map((v) => `{{${v}}}`).join(', ')}</strong> is used as the Bearer token.
                  You'll configure how it's obtained on the next step.
                </div>
              )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
              ...
              <div style={{ padding: '16px 20px', borderTop: '1px solid #E2E8F0', display: 'flex', flexDirection: 'column', gap: 12 }}>
                <AuthSection
                  ...
                  tokenMode={tokenMode} setTokenMode={setTokenMode}
                  staticToken={staticToken} setStaticToken={setStaticToken}
                  authEndpointUrl={authEndpointUrl} setAuthEndpointUrl={setAuthEndpointUrl}
                  authEndpointMethod={authEndpointMethod} setAuthEndpointMethod={setAuthEndpointMethod}
                  authEndpointBody={authEndpointBody} setAuthEndpointBody={setAuthEndpointBody}
                  tokenResponsePath={tokenResponsePath} setTokenResponsePath={setTokenResponsePath}
                  authConnectorId={authConnectorId} setAuthConnectorId={setAuthConnectorId}
...
interface AuthSectionProps {
  ...
  tokenMode: 'static' | 'dynamic' | 'connector'; setTokenMode: (v: 'static' | 'dynamic' | 'connector') => void;
  staticToken: string; setStaticToken: (v: string) => void;
  authEndpointUrl: string; setAuthEndpointUrl: (v: string) => void;
  authEndpointMethod: string; setAuthEndpointMethod: (v: string) => void;
  authEndpointBody: string; setAuthEndpointBody: (v: string) => void;
  tokenResponsePath: string; setTokenResponsePath: (v: string) => void;
  authConnectorId: string; setAuthConnectorId: (v: string) => void;
...
const TOKEN_MODES = [
  ...
  { value: 'dynamic', label: 'Login endpoint' },
  { value: 'connector', label: 'Use existing connector' },
] as const;

const AuthSection: React.FC<AuthSectionProps> = ({
  authType, setAuthType,
  tokenMode, setTokenMode,
  staticToken, setStaticToken,
  authEndpointUrl, setAuthEndpointUrl,
  authEndpointMethod, setAuthEndpointMethod,
  authEndpointBody, setAuthEndpointBody,
  tokenResponsePath, setTokenResponsePath,
  authConnectorId, setAuthConnectorId,
  ...
  const selectedAuthConnector = connectors.find((c) => c.id === authConnectorId);
        ...
        <div style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          {authType === 'Bearer' && (
            <>
                ...
                {TOKEN_MODES.map((m) => (
                  <button
                    ...
                    style={{
                      ...
                      fontSize: 11, fontWeight: tokenMode === m.value ? 600 : 400,
                      backgroundColor: tokenMode === m.value ? '#1D4ED8' : '#F8FAFC',
                      color: tokenMode === m.value ? '#fff' : '#64748B',
                      borderRight: m.value !== 'connector' ? '1px solid #E2E8F0' : 'none',
                      transition: 'all 80ms',
                    }}
                  >
                    {m.label}
              ...
              {tokenMode === 'dynamic' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10, padding: '10px 12px', backgroundColor: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: 6 }}>
                  <div style={{ display: 'flex', gap: 8 }}>
                    <div style={{ flex: '0 0 80px' }}>
                      <label style={labelStyle}>METHOD</label>
                      <select value={authEndpointMethod} onChange={(e) => setAuthEndpointMethod(e.target.value)} style={{ ...inputStyle, cursor: 'pointer' }}>
                        <option>POST</option><option>GET</option>
                      </select>
                    </div>
                    <div style={{ flex: 1 }}>
                      <label style={labelStyle}>LOGIN URL</label>
                      <input value={authEndpointUrl} onChange={(e) => setAuthEndpointUrl(e.target.value)} style={inputStyle} placeholder="https://api.example.com/login" />
                    </div>
                  </div>
                  <div>
                    <label style={labelStyle}>REQUEST BODY (JSON)</label>
                    <textarea value={authEndpointBody} onChange={(e) => setAuthEndpointBody(e.target.value)} rows={2}
                      style={{ ...inputStyle, height: 'auto', padding: '6px 10px', fontFamily: 'monospace', fontSize: 11, resize: 'vertical' }} />
                  </div>
                  <div>
                    <label style={labelStyle}>
                      TOKEN FIELD IN RESPONSE
                      <span style={{ fontWeight: 400, color: '#94A3B8', marginLeft: 6, fontSize: 10 }}>e.g. <code>data.token</code></span>
                    </label>
                    <input value={tokenResponsePath} onChange={(e) => setTokenResponsePath(e.target.value)} style={inputStyle} placeholder="token" />
                  </div>
                </div>
              )}
              ...
              {tokenMode === 'connector' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  <div>
                    <label style={labelStyle}>SELECT AUTH CONNECTOR</label>
                    <select
                      value={authConnectorId}
                      onChange={(e) => setAuthConnectorId(e.target.value)}
                      style={{ ...inputStyle, cursor: 'pointer' }}
                    >
